...
firstStop.js: who made the stop the pit question is about, and on which lap, off OpenF1 in the browser and cached in localStorage. teamOfQuestion (Admin uses it too), firstStopOf, stopLabel ("Haas, Ocon, Lap 14") and useFirstStop. Pit rows with over a minute in the lane are red-flag or start artefacts and are skipped; stop_duration is empty on 2026 rows, so the stop is the team's first by lap, and the time is always Andrew's from results. **"BOX BOX line" is called "Your Matchup's Line" everywhere since 2026-09-13**, footnoted "*Your Matchup's Line is the average of your four pit stop picks." in italic blue under both charts; the points are still BOX BOX points. On Home (BoxBoxScore in VegasHome.jsx) and the deck (BoxBoxStrip in Weekly.jsx) the stop is a big dot, green on your side, pink on theirs, blue on a push.
...
teamTable.js: the team game computed. Pure, no React and no Supabase, so any page can render a standings row without growing a second copy of the scoring rules. buildTeamTable, rankByAverage, nextFixtures, TIEBREAKS. Also buildTeamPower, the team Power Index: weights set by Andrew 2026-09-09, 30% season scoring average, 20% last five, 20% last two, 20% wins in the last five with a draw as half, 10% strength of schedule stretched so the easiest run of opponents in the league is 0 and the hardest is 10. A perfect week is 100 points, fixed, and the five parts make a raw mark against it. **The rating is that mark graded on a curve**, set 2026-09-09 when the raw marks ran 82 to 45 and Andrew wanted teams in the low 90s and teams in the F range: the league average is 75 and every 10 points is one standard deviation of the league that week, so the number reads like a grade and is relative to the league. The curve reorders nobody. `raw` stays on the row. Built for this round and the one before so a place carries its move. Ranked across all 24 teams, never within a division.
...
PowerPage.jsx: the team Power Rankings at /power, on the Vegas look, reached from the More page. Same skeleton as TeamsPage: one row a team with place, move since last round, division tag, record, last five and the rating out of 100, then a write-up on every team. PowerBoard is the presentational half, so scripts/smoke-power.jsx renders it without a network. Computes nothing.
powerNotes.js: the write-ups, one short paragraph a team the way ESPN runs theirs. Pure. buildPowerNotes(buildTeamPower(db), db). A headline off Andrew's TEAM_PUN bank in wire.js, the number with its rank, one colour sentence when the week has one (last time out, a teammate gap, or a schedule at either end of the league), and who is next. No lore and no restating the place or the move, which the row above already shows; cut to half on 2026-09-09. Phrasing choices are hashed off team and round, never random. Read the output with npm run peek:power before touching a sentence.
...
picks.auto: true when the randomizer made the picks, Admin's generator and Fernolo both. Added 2026-09-25 by scripts/picks_auto_column.sql, with scripts/picks_auto_backfill.sql marking the ten rows made before the column existed (found by submitted_at after the deadline; round 11's six were hand-entered for players who had picked, Andrew's word, and stay unmarked). Shown as **Fernolo's seal**, src/FernoloSeal.jsx drawing public/fernolo-seal.png (Andrew's neon "Picked by Fernolo Bort" artwork, cropped square with sips, 360px, drawn opaque with its black disc like a sticker; it shipped screen-blended and Andrew's read was too transparent and not clear): 56px beside the face on the Home matchup card, three quarters of the face hung off its upper right on the four-hand board (Home and the deck's matchup press; the name plate paints above it), and 42px beside the whole score block on /players in every mode but Trophies, read off the picks for the last scored race. **Bigger, opaque, clearer** were the three corrections on 2026-09-25, in that order. /schedule has no faces, so there the name is pink with a legend line under the round header. The seal is a picture and not legible at those sizes; every use carries a title. The schedule's pick read falls back to a select without the column if it is missing, so the code can ship before the SQL runs; Home and the deck select * and never needed the guard.
