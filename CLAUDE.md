...
powerNotes.js: the write-ups, one short paragraph a team the way ESPN runs theirs. Pure. buildPowerNotes(buildTeamPower(db), db). Where the team sits and why, last time out when the result says something, who is carrying whom, the schedule at either end of the league, and who is next. Phrasing choices are hashed off team and round, never random. Read the output with npm run peek:power before touching a sentence.
