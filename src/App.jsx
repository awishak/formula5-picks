...
const PREVIEW_SLIDES = [
  ...
  { title: "Your Matchup’s Line", emoji: "📦", body: "Each matchup has a line, Your Matchup’s Line — the average of all 4 players' pit guesses. One team is OVER, one is UNDER. Right side = +5 team pts. Wrong side = −1. Six-point swing." },
  { title: "The Tension", emoji: "🤔", body: "Guess accurately → maximize your individual score. Guess strategically → shift Your Matchup’s Line for your team. You can't always do both. That's what makes it fun." },
...
const ROUTES = [
  ...
  { path: "/hands/7", page: "hands7" },
  { path: "/hands/8", page: "hands8" },
...
export default function App() {
  ...
  const ideaN = /^hands([1-8])$/.exec(activePage);
  ...
  return (
    <>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Geologica:wght@300;400;700;900&family=DM+Sans:wght@300;400;500;600;700&display=swap'); * { box-sizing: border-box; margin: 0; padding: 0; } body { background: ${BG}; } body.v-dark { background: #07070c; } .app-wrap { max-width: 480px; margin: 0 auto; min-height: 100vh; background: ${BG}; padding-bottom: 80px; }`}</style>
      ...
      <div className="app-wrap" style={{
        ...
        // Admin is a tool with tables in it, used at a desk. 480px is right for
        // every page somebody reads on a phone and wrong for this one.
        ...(activePage === "admin" ? { maxWidth: 900 } : {}),
      }}>
        ...
        {activePage === "admin" && (adminUnlocked ? <Admin /> : (
          <div style={{ padding: "60px 20px", textAlign: "center", background: V.bg,
            minHeight: "100dvh" }}>
            <p style={{ fontFamily: VFD, fontWeight: 700, fontSize: 22, color: V.text, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 8 }}>Admin Access</p>
            <p style={{ fontFamily: VFB, fontSize: 14, color: V.text2, marginBottom: 20 }}>Enter the access code to continue</p>
            <input type="password" placeholder="Enter code…" value={adminCode}
              ...
              style={{ width: "100%", maxWidth: 240, padding: "12px 16px", borderRadius: 10, border: `1px solid ${V.border2}`, background: V.bg3, color: V.text, fontFamily: VFB, fontSize: 15, textAlign: "center", outline: "none" }}
            />
            {adminCode.length > 0 && adminCode.toLowerCase() !== "stroll" && (
              <p style={{ fontFamily: VFB, fontSize: 13, color: V.pink, marginTop: 10 }}>Incorrect code</p>
            )}
          </div>
        ))}
