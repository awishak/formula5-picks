...
// Recap rendered inline — no separate import needed
...
function RecapPage() {
  ...
  const TEXT2 = "#6b6b80", DARK = "#1e1e2a", BLUEDARK = "#2a6fa8", BLUE = "#6cb8e0", BORDER = "#d8d2c4", GOLD = "#c9a820", SILVER = "#a0a0a0", RED = "#e04a4a", TEXT = "#1e1e2a";
  const FD = "'Geologica', sans-serif", FB = "'DM Sans', sans-serif";
  ...
  if (recaps.length === 0) return (
    <div style={{ padding: "20px 20px 100px" }}>
      <p style={{ fontFamily: FD, fontWeight: 900, fontSize: 22, color: DARK, textTransform: "uppercase", margin: "0 0 4px" }}>Race Recaps</p>
    ...
    <div style={{ padding: "20px 20px 100px" }}>
      ...
      <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4, scrollbarWidth: "none", marginBottom: 20 }}>
        {races.map(function(r) {
          var has = roundsWithRecaps[r.round];
          return (
            <button key={r.round} onClick={function() { if (has) setActiveRound(r.round); }} style={{
              flexShrink: 0, width: 36, height: 36, borderRadius: "50%",
              border: "1px solid " + (activeRound === r.round ? DARK : has ? BORDER : BORDER + "50"),
              background: activeRound === r.round ? DARK : "transparent",
              color: activeRound === r.round ? "#fff" : has ? TEXT2 : BORDER,
      ...
      {activeRecap ? (
        <div>
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid " + BORDER, padding: "20px 18px", marginBottom: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
              <span style={{ fontSize: 18 }}>📝</span>
              <p style={{ fontFamily: FD, fontWeight: 800, fontSize: 14, color: DARK, textTransform: "uppercase", margin: 0 }}>League Recap</p>
            </div>
            {activeRecap.league_recap.split("\n\n").map(function(para, idx) {
              return <p key={idx} style={{ fontFamily: FB, fontSize: 14, color: TEXT, lineHeight: 1.7, margin: idx === 0 ? 0 : "12px 0 0" }}>{para}</p>;
            })}
          </div>
...
function HomePage({ currentUser, onNavigate, onChangeName, onSelectName }) {
  ...
  let avatarHash = 0;
  for (let i = 0; i < (currentUser || "").length; i++) avatarHash = (currentUser || "").charCodeAt(i) + ((avatarHash << 5) - avatarHash);
  const avatarHue = (Math.abs(avatarHash) * 137) % 360;
  const avatarBg = `hsl(${avatarHue}, 50%, 60%)`;
