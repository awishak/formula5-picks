...
// Round whose clip plays in front of the recap deck. 14 is the Spanish Grand
// Prix at Ifema Madrid (the season has two races under that name; 7 is
...
export default function App() {
  ...
  const [raceClipOn, setRaceClipOn] = useState(false);
  ...
  // The race clip in front of the deck is gated on round 14 being scored, so it
  // appears when the results are revealed and not before.
  //
  // The check lives here, not in Recap.jsx: that file runs entirely on
  ...
  useEffect(() => {
    async function checkRaceClip() {
      try {
        ...
        if (!race) return;
        const { data: scored } = await supabase.from("scores")
          .select("id").eq("race_id", race.id).limit(1);
        setRaceClipOn(!!(scored && scored.length));
      } catch (e) { /* silent — no clip rather than no deck */ }
    }
    checkRaceClip();
  }, []);
