...
function useOpenF1Drivers() {
  var [driverMap, setDriverMap] = useState(new Map());
  useEffect(function () {
    var cancelled = false;
    async function fetchDrivers() {
      try {
        ...
        Object.entries(F1_TEAMS_FALLBACK).forEach(function (e) {
          fallback.set(e[0], { team: e[1], headshot: null, teamColor: null });
        });
        setDriverMap(fallback);
      }
    }
    fetchDrivers();
...
function findDriver(dm, name) {
  if (!name || dm.size === 0) return { team: F1_TEAMS_FALLBACK[name] || "", headshot: null, teamColor: BLUE };
  if (dm.has(name)) return dm.get(name);
  var last = name.split(" ").pop().toLowerCase();
  for (var entry of dm) { if (entry[0].split(" ").pop().toLowerCase() === last) return entry[1]; }
  for (var entry2 of dm) { if (entry2[0].toLowerCase().includes(name.toLowerCase()) || name.toLowerCase().includes(entry2[0].toLowerCase())) return entry2[1]; }
  return { team: F1_TEAMS_FALLBACK[name] || "", headshot: null, teamColor: BLUE };
}

function shortName(n) { if (!n) return "?"; var p = n.split(" "); return p.length < 2 ? n : p[0][0] + ". " + p.slice(1).join(" "); }
function lastName(n) { if (!n) return "?"; return n.split(" ").pop(); }
...
export default function PickIntel({ currentUser }) {
  ...
  useEffect(function () {
    async function load() {
      ...
      setRaces(racesData); setPlayers(playersData); setPicks(picksData); setTeams(teamsData); setSchedule(scheduleData);
  ...
  var myBestFinishRaw = myPick ? String(myPick.best_finish || "").replace(/[^0-9]/g, "") : "";
  var myBestFinish = myBestFinishRaw ? parseInt(myBestFinishRaw, 10) : null;
  ...
  function PicksTab() {
    ...
    racePicks.forEach(function (pk) {
      ...
      if (pk.best_finish != null && pk.best_finish !== "") {
        var bfNum = parseInt(String(pk.best_finish).replace(/[^0-9]/g, ""), 10);
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
          {topSorted.map(function (e) { return <Card key={e[0]} name={e[0]} count={e[1]} isMine={e[0] === myTopPick} />; })}
        </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
          {midSorted.map(function (e) { return <Card key={e[0]} name={e[0]} count={e[1]} isMine={myDrivers.indexOf(e[0]) >= 0 && e[0] !== myTopPick} />; })}
        </div>
  ...
  function LineupsTab() {
    ...
    var combosFiltered = comboSorted.filter(function (e) { return e[1] >= 2; });
    ...
    var consensus = Object.entries(orders).filter(function (e) { return e[1] >= 2; }).sort(function (a, b) { return b[1] - a[1]; });
      ...
      <div>
        ...
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid " + BORDER, padding: "12px 14px", marginBottom: 20 }}>
          ...
          {combosFiltered.map(function (entry, i) {
            var isMyComboRow = myComboKey === entry[0];
            return (
        ...
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid " + BORDER, padding: "12px 14px" }}>
          ...
          {consensus.map(function (entry, i) {
            var isMyOrderRow = myOrderKey === entry[0];
            return (
  ...
  function AllPicksTab() {
    ...
    rows.sort(function (a, b) {
      ...
      if (sortCol === "best") {
        var va = parseInt(String(a.best_finish || "99").replace(/[^0-9]/g, ""), 10) || 99;
        var vb = parseInt(String(b.best_finish || "99").replace(/[^0-9]/g, ""), 10) || 99;
        return sortAsc ? va - vb : vb - va;
      }
      if (sortCol === "pit") {
        var pa = a.pit_guess != null ? Number(a.pit_guess) : 99;
        var pb = b.pit_guess != null ? Number(b.pit_guess) : 99;
              ...
              {rows.map(function (pk, i) {
                ...
                return (
                  <tr key={pk.player_id} style={{ background: isMe ? BLUE + "10" : i % 2 === 0 ? "#fff" : DARK + "03", borderBottom: "1px solid " + BORDER + "30" }}>
                    ...
                    <td style={Object.assign({}, tdStyle, { fontFamily: FD, fontWeight: 700, color: BLUEDARK })}>{formatBf(pk.best_finish)}</td>
